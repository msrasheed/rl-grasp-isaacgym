...
class HandCamBoxEnv:
  ...
  def step(self, dof_targets):
    ...
    if reset_envs.numel() != 0:
      ...
      self.gym.set_dof_state_tensor_indexed(self.sim, gymtorch.unwrap_tensor(self.reset_dof_state_tensor), gymtorch.unwrap_tensor(dar_int32), len(dar_int32))

      actor_resets = self.env_to_actors[reset_envs]
      ar_int32 = actor_resets.to(device=self.device, dtype=torch.int32)
      self.gym.set_actor_root_state_tensor_indexed(self.sim, gymtorch.unwrap_tensor(self.reset_root_body_tensor), gymtorch.unwrap_tensor(ar_int32), len(ar_int32))
